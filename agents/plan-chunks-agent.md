...
**Then follow the story's needs by issuing the ranged reads the slice hands you.** Each symbol in the slice carries a `[off=N,lim=M]` annotation - the exact parameters for a targeted `Read(<file>, offset=N, limit=M)`. Issue that ranged Read verbatim; do not open the whole file. Prefer the NARROWEST span that covers what you need - the method or member annotation, not the enclosing class annotation; read a class span only when you genuinely need the whole type. If you need more than a span shows, read the ADJACENT span (the sibling annotation above or below), never widen to the whole file. For a file's imports/usings, issue one bounded header read from line 1 up to the first annotated span. A symbol printed with NO `[off,lim]` annotation has no reliable span - only those fall back to a whole-file read. Files the slice does not cover at all are read normally. Gather patterns, dependencies, risks, and test examples as you go, in one pass.
...
**Infrastructure before features.** If a story needs new dependencies, test setup, or build config, put that in chunk 1. Don't plan "set up vitest" as chunk 4 when chunks 1-3 write code that needs testing. The extreme case is a project with NO runnable skeleton at all - there, chunk 1 is a Scaffold chunk:
  - **Detect greenfield with a Glob, not an impression.** Run a Glob at the story's target root (the project root you derived above - never the monorepo root) for a project manifest (`package.json`, `Cargo.toml`, `go.mod`, `pyproject.toml`), a build config, or a framework entry file. Only when that Glob returns genuinely nothing is the project un-scaffolded. Your Phase 1.3 research narrative does not decide this - partial scaffolds (a create-next-app with only the default page, a monorepo whose sibling package has build config) are exactly where an impression rounds the wrong way.
  - **The Scaffold chunk stands up the framework named in project.md** - dependencies, config, entry point - so every later chunk builds on a runnable skeleton.
  - **When the story carries a `mockup:` backlink, the mockup seeds the scaffold.** Drop mockup.html in as the base route/template: the finalist's markup and CSS ported VERBATIM, syntax translated to the framework's route format (a no-op for static HTML), values untouched - the approved page becomes the skeleton's first commit, not a re-typed screenshot. Wire tokens.yaml so the design bible enforces it from chunk 1.
  - **Never scaffold a project that already has a skeleton.** If the Glob returns anything, no Scaffold chunk - a false-positive re-bootstrap lands on a live codebase.
