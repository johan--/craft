...
<!-- DO NOT SIMPLIFY THIS SCHEMA. Mockup records anchor agent recovery, graduation
     ...
     tweak port) and by recovery - the material's identity ports onward, never
     the inlined bytes. -->
...
**If "Go from what's on disk":** continue - the funnel below proceeds as written.
...
## Brief
[vibe answer - or full muse briefing (all 3 directions) when the muse path ran - constraints loaded, mobile verdict]
...
## Materials
[orchestrator-fetched material ONLY - normally empty; one line per fetch:
 what (typeface family/weights or icon set), source, which round/direction]
```
...
1. **Load the constraints** - `.craft/design/tokens.yaml` and `.craft/design/locked.md` (which carries any design-vibe soul statement). These are think-with, not bible: deliberate breaks are licensed, announced when crossed, and tracked toward `## New Values` for the solidify beat. **When either file is absent (the cold path), there are simply no constraints to load** - assemble the brief from the vibe beat's outcome (when the muse path ran, the full muse briefing verbatim - all 3 directions; otherwise the vibe answer) and the alchemist's own reading of the surrounding code. Never fabricate constraints or import template values; no default palette ever reaches a brief.
...
**Tool requests (material the alchemist can't fake).** The alchemist's report may carry lines beginning with the exact marker `Tool request:` - naming a typeface (family + weights) or an icon set, and why the faked version would distort this round's reaction. No marker, no flow: skip everything below and proceed as always. When present, the orchestrator fetches the asset files itself - the alchemist never touches the network - and only ever from the pinned sources: Google Fonts for typefaces; for icons, the jsDelivr CDN mirror of the official Lucide, Phosphor, or Heroicons packages (`cdn.jsdelivr.net/npm/lucide-static/icons/<name>.svg` and the Phosphor/Heroicons npm equivalents - the same files the official repos publish, served from an origin built for repeated fetches). Never loop per-file over raw.githubusercontent.com - GitHub throttles rapid sequential raw fetches and the round grinds to a crawl. No other origin is ever fetched from - never hunt the web for font or icon files. Fetched files land in the mockup folder's `assets/` subdirectory, beside mockup.html - inside the mockup's `.craft/` write scope on both warm and cold paths - and fetch intermediates (raw source CSS, temp files) live in `assets/` too, never the mockup root. The orchestrator then wires the material into the page itself: injecting the base64 `@font-face` (or SVG markup) directly into mockup.html and leading the relevant display stacks with the new family is byte-plumbing, not design, and doing it fetch-side spares the alchemist tens of KB of base64 in its context. The next brief simply names the family now available; the alchemist designs with it. The page still makes zero requests at load and still works when the folder moves.

**The fetch is silent, disclosed, and bounded.** No AskUserQuestion and no conversational ask - the taste budget stays at most three, and a fetch into the mockup's own folder is logistics, not taste. Disclose it in one line when the round is presented ("fetched Inter from Google Fonts for direction B") and log one line per fetch in record.md `## Materials` (what, source, which round/direction asked). A request for anything that is not a typeface or icon set is declined in one plain line and the alchemist improvises - charts, animation, and engines stay hand-built, where the constraint does real work. Offline, a wholly failed fetch, or a licensed face that no open source carries degrades silently: the round proceeds on the alchemist's substitute exactly as before, with one honest line where it matters ("Söhne is licensed - using a tuned system stack") - never an error state, never a dead round. Silent degradation covers the whole-fetch failure only - a partial icon fetch is not silent. When one name in a set 404s, first check whether the set renamed it (icon sets deprecate names - Lucide's per-icon JSON metadata lists old names under `aliases`); a found rename self-heals and the fetch proceeds under the current name. Only a genuinely unknown name reaches the user, folded into the same disclosure line ("Lucide has no 'more-horizontal' - renamed to 'ellipsis'; fetched 17/18") - never a quiet count that ends short.
...
The pick becomes the base; brief the alchemist with variations of it. Hybrids are legal briefs ("B with C's cards"). **Options are Refine's default shape - converging to one is never inferred.** The round comes back as variations of the base unless the user explicitly asked for a single build. When a reaction hints at one direction in any way ("just build C as the full page"), confirm before briefing - one conversational line, asked as if you're not certain ("One converged take this round, or options again?") - never an AskUserQuestion, never a silent assumption. A scope expansion ("do the whole page, starting with C") is not a convergence request; a new user who gets one page where options should be doesn't know to ask where the rest went. Same mechanics: archive the outgoing round, then EDIT the living page toward the new round - never a from-scratch rewrite; the surrounding context and base CSS that didn't change are not regenerated - then verify, show, collect the reaction conversationally, record verbatim - the settle gate applies here exactly as in Diverge. Mark Refine complete.
...
- **Tweak:** the handoff brief states "direction pre-settled, converged mockup at [path]" so adhoc's classification doesn't re-open exploration. When the record carries an `origin`, the brief ALSO forwards it ("grew from [origin]") so the ported tweak can stamp its `grew_from` - the read side that keeps lineage alive on the tweak ramp. When `## Materials` is non-empty, the brief forwards the materials spec too: the direction uses real material the port must acquire through the project's idiom, never by porting the inlined base64.
- **Story:** story-new's "From mockup" source (commands/references/story-from-mockup.md) does the pre-fill. The mockup's CSS is NORMATIVE there - ported, never reinterpreted. It also reads `## Materials` and forwards the material spec into the produced story.
